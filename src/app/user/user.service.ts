import { SQL, and, count, gte, ilike, inArray, lte } from "drizzle-orm";
import { StatusCodes } from "http-status-codes";
import { type Json2CsvOptions, json2csv } from "json-2-csv";

import AuthenticationService from "@/app/authentication/authentication.service";

import PaginationManager from "@/core/pagination";
import DrizzleService from "@/databases/drizzle/service";
import type { RoleType, UserSchemaType } from "@/databases/drizzle/types";
...
interface RetrieveAllUserParams {
	sortBy?: string;
	sortOrder?: string;
	where?: SQL<unknown>;
}
...
export default class UserService extends DrizzleService {
	...
	async retrieveUsers(
		...
		try {
			...
			if (!filter.page || !filter.limit) {
				return await this.retrieveAllUsers({
					where: whereClause,
	...
	async exportUsersToCSV(): Promise<ServiceApiResponse<string>> {
		try {
			const usersResponse = await this.retrieveAllUsers({
				sortBy: "asc",
				sortOrder: "createdAt"
			});
